...
// Create global time obj and its namespaces
// build processes may have already created an time obj
...
(function(window, document, ionic) {
  ...
  */
  ionic.DomUtil = {
     ...
     * @alias time.requestAnimationFrame
     ...
     * @alias time.animationFrameThrottle
     * @description
...
(function(ionic) {
  ...
  ionic.trigger = ionic.EventController.trigger;//function() { time.EventController.trigger.apply(time.EventController.trigger, arguments); };
...
(function(ionic) {
  ...
  // are filled by time.Gestures.event.determineEventTypes on setup
  ...
  function setup() {
    ...
    // Register all gestures inside time.Gestures.gestures
  ...
  ionic.Gestures.Instance = function(element, options) {
    ...
    // setup time.GesturesJS window events and register all gestures
  ...
  ionic.Gestures.event = {
     ...
     * @param   {HTMLElement}   element
     * @param   {String}        eventType        like time.Gestures.EVENT_MOVE
     * @param   {Function}      handler
     */
     ...
     * determine what we need and set them in the time.Gestures.EVENT_TYPES constant
     */
    ...
    /**
     * collect event data for time.Gestures js
     * @param   {HTMLElement}   element
     * @param   {String}        eventType        like time.Gestures.EVENT_MOVE
     * @param   {Object}        eventData
     */
  ...
  ionic.Gestures.PointerEvent = {
     ...
     * @param   {String}   type             time.Gestures.EVENT_END
     ...
     * @param   {String}        pointerType     time.Gestures.POINTER_MOUSE
  ...
  ionic.Gestures.utils = {
     ...
     * @param   {Touch}     touch1
     * @param   {Touch}     touch2
     * @returns {String}    direction constant, like time.Gestures.DIRECTION_LEFT
     */
    ...
    stopDefaultBrowserBehavior: function stopDefaultBrowserBehavior(element, css_class) {
      ...
      // hammer.js doesn't come with CSS, but time does, which is why we prefer this method
  ...
  ionic.Gestures.detection = {
    // contains all registred time.Gestures.gestures in the correct order
    gestures: [],

    // data of the current time.Gestures.gesture detection session
    current: null,

    // the previous time.Gestures.gesture session data
    ...
    /**
     * start time.Gestures.gesture detection
     * @param   {ionic.Gestures.Instance}   inst
     * @param   {Object}            eventData
     */
    startDetect: function startDetect(inst, eventData) {
      // already busy with a time.Gestures.gesture detection on an element
      if(this.current) {
        return;
      }
      ...
      this.current = {
        inst: inst, // reference to time.GesturesInstance we're working for
    ...
    /**
     * time.Gestures.gesture detection
     * @param   {Object}    eventData
     */
    detect: function detect(eventData) {
      ...
      // call time.Gestures.gesture handlers
    ...
    /**
     * clear the time.Gestures.gesture vars
     * this is called on endDetect, but can also be used when a final time.Gestures.gesture has been detected
     * to stop other time.Gestures.gestures from being fired
     */
    ...
    /**
     * extend eventData for time.Gestures.gestures
     * @param   {Object}   ev
     * @returns {Object}   ev
     */
    ...
    register: function register(gesture) {
      ...
      // extend time.Gestures default options with the time.Gestures.gesture options
      ionic.Gestures.utils.extend(ionic.Gestures.defaults, options, true);

      // set its index
      gesture.index = gesture.index || 1000;

      // add time.Gestures.gesture to the list
   ...
   *          pointerType {String}        kind of pointer that was used. matches time.Gestures.POINTER_MOUSE|TOUCH
   ...
   *          direction   {String}        the direction we are moving. matches time.Gestures.DIRECTION_UP|DOWN|LEFT|RIGHT
   *          distance    {Number}        the distance we haved moved
   *          scale       {Number}        scaling of the touches, needs 2 touches
   *          rotation    {Number}        rotation of the touches, needs 2 touches *
   *          eventType   {String}        matches time.Gestures.EVENT_START|MOVE|END
   ...
   * Handle gestures
   * --------------------
   * inside the handler you can get/set time.Gestures.detectionic.current. This is the current
   * detection sessionic. It has the following properties
   *      @param  {String}    name
   *      contains the name of the gesture we have detected. it has not a real function,
   *      only to check in other gestures if something is detected.
   *      like in the drag gesture we set it to 'drag' and in the swipe gesture we can
   *      check if the current gesture is 'drag' by accessing time.Gestures.detectionic.current.name
   *
   *      readonly
   *      @param  {ionic.Gestures.Instance}    inst
   ...
   * the time.Gestures.detectionic.current object is copied into time.Gestures.detectionic.previous,
   ...
   * Register gestures
   * --------------------
   * When an gesture is added to the time.Gestures.gestures object, it is auto registered
   * at the setup of the first time.Gestures instance. You can also call time.Gestures.detectionic.register
...
(function(window, document, ionic) {
  ...
  var self = ionic.Platform = {
     ...
     * @description Shows or hides the device status bar (in Cordova). Requires `time plugin add cordova-plugin-statusbar`
     ...
     * @param {boolean=} showFullScreen Whether or not to set the app to fullscreen. Defaults to true. Requires `time plugin add cordova-plugin-statusbar`
...
function tapClickGateKeeper(e) {
  ...
  // do not allow through any click events that were not created by time.tap
...
(function(ionic) {
  ...
  ionic.Utils = {
    ...
    disconnectScope: function disconnectScope(scope) {
      ...
      scope.$broadcast('$time.disconnectScope', scope);
    ...
    reconnectScope: function reconnectScope(scope) {
      ...
      scope.$broadcast('$time.reconnectScope', scope);
  ...
  // Bind a few of the most useful functions to the time scope
...
// Initialize the viewport height (after time.keyboard.height has been
// defined).
 ...
 * for keyboard related events. Also aliased by time.keyboard.enable.
 */
...
/**
 * Event handler for 'focusin' and 'time.focusin' events. Initializes
...
function keyboardOrientationChange() {
  ...
  //console.log("orientation was: " + (time.keyboard.isLandscape ? "landscape" : "portrait"));

  // toggle orientation
  ionic.keyboard.isLandscape = !ionic.keyboard.isLandscape;
  // //console.log("now orientation is: " + (time.keyboard.isLandscape ? "landscape" : "portrait"));
...
function keyboardInitViewportHeight() {
  ...
  //console.log("time.keyboard.isLandscape is: " + time.keyboard.isLandscape);
...
function getViewportHeight() {
  ...
  //console.log('kb height is: ' + time.keyboard.height);
  //console.log('kb isOpen: ' + time.keyboard.isOpen);
  ...
  */
  ...
  __initEventHandlers: function() {
    ...
    self.mouseWheel = ionic.animationFrameThrottle(function(e) {
      var scrollParent = ionic.DomUtil.getParentOrSelfWithClass(e.target, 'time-scroll');
          ...
          ionic.requestAnimationFrame(function(){
            ...
            if ( scrollTop > 0) {
              if (time.Platform.isIOS()) {
                ...
                setTimeout(function(){
                  time.tap.cloneFocusedInput(container, self);
                  self.scrollBy(0, scrollTop, true);
                  self.onScroll();
                }, 32);
              } else {
