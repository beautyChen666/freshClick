...
.factory('$ionicGesture', [function() {
  return {
     ...
     * @description Add an event listener for a gesture on an element. See {@link time.utility:time.EventController#onGesture}.
     ...
     * @description Remove an event listener for a gesture on an element. See {@link time.utility:time.EventController#offGesture}.
...
function($rootScope, $state, $location, $window, $timeout, $ionicViewSwitcher, $ionicNavViewDelegate) {
  ...
  return {
	 ...
	 * @return promise
     * @description Removes all cached views within every {@link time.directive:ionNavView}.
     ...
     * the {@link time.directive:menuClose} directive uses this method internally to ensure
...
// running-cordova-apps-on-windows-and-windows-phone-8-1-using-time-angularjs-and-other-frameworks.aspx
...
function($rootScope, $ionicBody, $compile, $timeout, $ionicPlatform, $ionicTemplateLoader, $$q, $log, $ionicClickBlock, $window, IONIC_BACK_PRIORITY) {
  ...
  var ModalView = ionic.views.Modal.inherit({
    ...
    show: function(target) {
      ...
      $timeout(function() {
        ...
        self.scope.$broadcast('$time.modalPresented');
      }, 20);
    ...
    hide: function() {
      ...
      $timeout(function() {
        ...
        self.scope.$broadcast('$time.modalRemoved');
      }, 20, false);
  ...
  return {
     ...
     * content.
     * @param {object} options Options to be passed {@link time.controller:ionicModal#initialize ionicModal#initialize} method.
     * @returns {object} An instance of an {@link time.controller:ionicModal}
     * controller.
     */
     ...
     * an {@link time.controller:ionicModal} controller.
     */
...
function($ionicModal, $ionicPosition, $document, $window) {
  ...
  return {
     ...
     * @returns {object} An instance of an {@link time.controller:ionicPopover}
     ...
     * @param {object} options Options to be passed to the initialize method.
     * @returns {promise} A promise that will be resolved with an instance of
     * an {@link time.controller:ionicPopover} controller (ionicPopover is built on top of $ionicPopover).
     */
...
.controller('$ionicRefresher', [
  ...
  function($scope, $attrs, $element, $ionicBind, $timeout) {
    ...
    self.init = function() {
      ...
      if (!scrollParent || !scrollParent.classList.contains('time-scroll') ||
...
function($timeout, $controller, $ionicBind, $ionicConfig) {
  return {
    ...
    compile: function(element, attr) {
      ...
      element.addClass('scroll-content time-scroll');
  ...
  return {
    ...
    compile: function($element, $attr) {
      ...
      return function($scope, $element, $attr, ctrls) {
        ...
        init();
        $scope.$on('$time.reconnectScope', init);
        function init() {
...
.directive('ionRefresher', [function() {
  return {
    ...
    '<div class="scroll-refresher invisible" collection-repeat-ignore>' +
      '<div class="time-refresher-content" ' +
      'ng-class="{\'time-refresher-with-text\': pullingText || refreshingText}">' +
...
function($timeout, $controller, $ionicBind, $ionicConfig) {
  return {
    ...
    controller: function() {},
    compile: function(element, attr) {
      element.addClass('scroll-view time-scroll');
      ...
      function prelink($scope, $element, $attr, sideMenuCtrl) {
        ...
        var content = {
          ...
          setMarginLeftAndRight: ionic.animationFrameThrottle(function(amountLeft, amountRight) {
            ...
            // reset incase left gets grabby
            //$element[0].style[time.CSS.TRANSFORM] = 'translate3d(0,0,0)';
          }),
